...
(function () {
	...
	$(function () {
		...
		// 0504待確認
		function returnMsg(){
			strEmail = $('#emailSubscribe').val();
			if (strEmail.search(/[\w-]+@([\w-]+\.)+[\w-]+/) != -1){
			  $('.messageSubscribe').slideDown().delay(3000).slideUp();
			  $('.validateSubscribe').hide();
			  $('#emailSubscribe').val('');
			}else{
			  $('.messageSubscribe').hide();
			  $('.validateSubscribe').show();
			}
			return false;
		  }
		  // 0504待確認
				...
				$form.attr('novalidate', 'novalidate').ajaxForm({
					...
					beforeSubmit: function (arr, $form, options) {
						...
						if (isValidated(inputs, captcha)) {
							...
							if (output.hasClass("snackbars")) {
								output.html('<p><span class="icon text-middle fa fa-circle-o-notch fa-spin icon-xxs"></span><span>You have subscribed. Thank you!</span></p>');
								output.addClass("active");
							}
						} else {
							return false;
